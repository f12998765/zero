...
public class LinkServiceImpl implements LinkService {
    ...
    public List<User> getUserByJoinPro(int id) {
        List<Integer> list = this.userProMapper.selectUserId(id);
    ...
    public List<User> getUserByJoinTask(int id) {
        List<Integer> list = this.userTaskMapper.selectUserId(id);
        if(list.size()==0)return null;
        List<User> list1 = this.userMapper.selectAllById(list);
        return list1;
    }
    ...
    public List<User> getUserByJoinBug(int id) {
        List<Integer> list = this.userBugMapper.selectUserId(id);
        if(list.size()==0)return null;
        List<User> list1 = this.userMapper.selectAllById(list);
        return list1;
    }
    ...
    public List<Project> getProByJoinUser(int id) {
        List<Integer> list = this.userProMapper.selectByUserId(id);
        if(list.size()==0)return null;
    ...
    public List<Task> getTaskByJoinUser(int id) {
        List<Integer> list = this.userTaskMapper.selectByUserId(id);
        if(list.size()==0)return null;
    ...
    public List<Bug> getBugByJoinUser(int id) {
        List<Integer> list = this.userBugMapper.selectByUserId(id);
        if(list.size()==0)return null;
